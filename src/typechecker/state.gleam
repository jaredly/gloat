...
pub type StateData = #(Int, dict.Dict(String, types.Type))
  ...
  case list_ {
    ...
    [one, ..rest] ->
      bind(f(one), fn(mapped) {
        bind(map_list(rest, f), fn(rest_mapped) { pure([mapped, ..rest_mapped]) })
      })
  }
}
...
pub fn foldr_list(list_: List(a), init: b, f: fn(b, a) -> State(b)) -> State(b) {
  case list_ {
    [] -> pure(init)
    [one, ..rest] ->
      bind(foldr_list(rest, init, f), fn(next) { f(next, one) })
  }
}
...
pub fn each_list(list_: List(a), f: fn(a) -> State(Nil)) -> State(Nil) {
  case list_ {
    [] -> pure(Nil)
    [one, ..rest] ->
      bind(f(one), fn(_) { each_list(rest, f) })
  }
}

pub const empty_state: StateData = #(0, dict.new())

pub fn run_empty(state: State(a)) -> a {
  eval(state, empty_state)
}

pub fn next_idx() -> State(Int) {
  bind(get(), fn(#(idx, subst)) {
    bind(put(#(idx + 1, subst)), fn(_) { pure(idx) })
  })
}

pub fn get_subst() -> State(dict.Dict(String, types.Type)) {
  bind(get(), fn(#(_, subst)) { pure(subst) })
}
...
pub fn reset_subst(new_subst: dict.Dict(String, types.Type)) -> State(dict.Dict(String, types.Type)) {
  bind(get(), fn(#(idx, old_subst)) {
...
pub fn apply_with(f: fn(dict.Dict(String, types.Type), a) -> b, arg: a) -> State(b) {
