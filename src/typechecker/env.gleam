...
pub fn empty() -> TEnv {
  TEnv(dict.new(), dict.new(), dict.new(), dict.new())
}

pub fn merge(one: TEnv, two: TEnv) -> TEnv {
  let TEnv(v1, c1, t1, a1) = one
  let TEnv(v2, c2, t2, a2) = two
  TEnv(
    dict.merge(v1, v2),
    dict.merge(c1, c2),
    dict.merge(t1, t2),
...
pub fn with_type(env: TEnv, name: String, scheme_: scheme.Scheme) -> TEnv {
  let TEnv(values, tcons, types_, aliases) = env
  TEnv(dict.insert(values, name, scheme_), tcons, types_, aliases)
}

pub fn with_scope(env: TEnv, scope: dict.Dict(String, scheme.Scheme)) -> TEnv {
  let TEnv(values, tcons, types_, aliases) = env
  TEnv(dict.merge(values, scope), tcons, types_, aliases)
}

pub fn resolve(env: TEnv, name: String) -> Result(scheme.Scheme, Nil) {
  let TEnv(values, _, _, _) = env
...
pub fn apply(subst: types.Subst, env: TEnv) -> TEnv {
  let TEnv(values, tcons, types_, aliases) = env
  TEnv(scope_apply(subst, values), tcons, types_, aliases)
}
