...
fn list(
  ...
  case tokens {
    ...
    [#(t.DotDot, _), ..tokens] -> {
      ...
      use #(end, tokens) <- result.try(right_square_with_optional_comma(tokens))
    ...
    _ -> {
      ...
      case tokens {
        ...
        [#(t.Comma, _), #(t.DotDot, _), ..tokens] -> {
          use #(rest, tokens) <- result.try(parser(tokens))
          use #(end, tokens) <- result.try(right_square_with_optional_comma(
            tokens,
          ))
          Ok(ParsedList(list.reverse(acc), Some(rest), tokens, end + 1))
        }
...
fn right_square_with_optional_comma(
  tokens: Tokens,
) -> Result(#(Int, Tokens), Error) {
  case tokens {
    [#(t.Comma, _), #(t.RightSquare, P(end)), ..tokens] -> Ok(#(end, tokens))
    _ -> {
