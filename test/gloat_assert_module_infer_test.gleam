...
fn assert_module_infer(code: String, expected: List(#(String, String))) {
  assert Ok(sort_pairs(expected)) == infer_module(code, [])
}
...
fn assert_module_infer_with_deps(
  ...
  assert Ok(sort_pairs(expected)) == infer_module(code, deps)
}
...
pub fn infer_module_test6_test() {
  assert_module_infer(
    ...
    [#("Singleton", "Singleton"), #("go", "fn(Singleton) -> Int")],
  )
}
...
pub fn infer_module_test7_test() {
  assert_module_infer(
    ...
    [#("Box", "fn(a) -> Box(a)"), #("unbox", "fn(Box(a)) -> a")],
  )
}
...
pub fn infer_module_test22_test() {
  assert_module_infer(
    "pub fn one() { 1 }\n         pub fn zero() { one() - 1 }\n         pub fn two() { one() + zero() }",
    [#("one", "fn() -> Int"), #("two", "fn() -> Int"), #("zero", "fn() -> Int")],
  )
}
...
pub fn fn_annotation_reused_test() {
  assert_module_infer(
    ...
    [#("Box", "fn(a) -> Box(a)"), #("go", "fn(Box(a)) -> fn(Box(a)) -> Bool")],
  )
  assert_module_infer(
...
pub fn box_record_test() {
  assert_module_infer(
    ...
    [#("Box", "fn(Nil, Int, Int, Int) -> Box"), #("main", "fn() -> Box")],
  )
}
...
pub fn custom_type_module_constants_test() {
  assert_module_infer("pub type Test { A }\n        pub const some_test = A", [
    #("A", "Test"),
    #("some_test", "Test"),
  ])
}
...
pub fn type_annotations_test() {
  assert_module_infer(
    ...
    [#("Box", "fn(String, a) -> Box(a)"), #("id", "fn(Box(a)) -> Box(a)")],
  )
  assert_module_infer("pub fn go(x: Int) { x }", [#("go", "fn(Int) -> Int")])
  assert_module_infer("pub fn go(x: b) -> b { x }", [#("go", "fn(a) -> a")])
  assert_module_infer("pub fn go(x) -> b { x }", [#("go", "fn(a) -> a")])
  assert_module_infer("pub fn go(x: b) { x }", [#("go", "fn(a) -> a")])
  ...
  assert_module_infer("pub fn go(x: b, y: c) { x }", [#("go", "fn(a, b) -> a")])
  assert_module_infer("pub fn go(x) -> Int { x }", [#("go", "fn(Int) -> Int")])
  assert_module_infer(
    "pub fn id(x: x) { x }\n         pub fn float() { id(1.0) }\n         pub fn int() { id(1) }",
    [
...
pub fn early_function_generalisation_test() {
  assert_module_infer("pub fn id(x) { x }\n         pub fn int() { id(1) }", [
    #("id", "fn(a) -> a"),
    #("int", "fn() -> Int"),
  ])
}
...
pub fn generic_inner_access_test() {
  assert_module_infer(
    ...
    [#("B", "fn(a) -> B(a)"), #("b_get_first", "fn(B(#(a))) -> a")],
  )
}
...
pub fn infer_label_shorthand_in_call_arg_test() {
  assert_module_infer(
    ...
    [#("main", "fn() -> Nil"), #("wibble", "fn(Int, Float, Bool) -> Nil")],
  )
}
...
pub fn infer_label_shorthand_in_constructor_arg_test() {
  assert_module_infer(
    ...
    [#("Wibble", "fn(Int, Bool, Float) -> Wibble"), #("main", "fn() -> Wibble")],
  )
}
...
pub fn infer_label_shorthand_in_pattern_arg_test() {
  assert_module_infer(
    ...
    [#("Wibble", "fn(Int, Bool, Int) -> Wibble"), #("main", "fn() -> Int")],
  )
}
